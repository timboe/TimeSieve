...
static void notifyUpdateProc(Layer *this_layer, GContext *ctx) {
  ...
  graphics_draw_text(ctx, itemName, fonts_get_system_font(FONT_KEY_GOTHIC_14_BOLD), GRect(b.origin.x+35, b.origin.y+15,b.size.w-40,30), GTextOverflowModeWordWrap, GTextAlignmentLeft, NULL);
}
...
void create_timeSieve_layer(Window* parentWindow) {
  ...
  s_notifyLayer = layer_create( GRect(window_bounds.origin.x+2, window_bounds.origin.y+2, window_bounds.size.w-4, window_bounds.size.h-4) ); // border 5 top and bottom
