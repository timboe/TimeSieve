...
static void sub_window_load(Window* parentWindow) {
  ...
  MenuLayer* s_subMenuLayer = menu_layer_create(layer_get_frame(window_get_root_layer(parentWindow)));
  //const int context = *((int*) window_get_user_data(parentWindow)); // TODO mem leaK?

  menu_layer_set_callbacks(s_subMenuLayer, /*window_get_user_data(parentWindow)*/ NULL, (MenuLayerCallbacks){ // TODO leak here? no...
  ...
  //layer_add_child(windowLayer, getNotifyLayer());//TODO memory leak from here?  NO
  //initBuyWindowRes(context); //TODO memory leak from here? NO
}

static void sub_window_unload(Window* parentWindow) {
  //destroyNotifyLayer();//TODO memory leak from here? NO
  //deinitBuyWindowRes(context);//TODO memory leak from here? NO
